...
import { db } from '../db';
import { componentKits, kitComponents, kitArtwork, insertComponentKitSchema, insertKitComponentSchema, insertKitArtworkSchema } from '@shared/schema';
...
// Setup artwork upload middleware to handle multiple files
...
}).array('artwork', 6); // Accept up to 6 files with field name 'artwork'

// Upload artwork for a kit
router.post('/kits/:kitId/artwork', isAdmin, (req: Request, res: Response) => {
  artworkUpload(req, res, async (err) => {
    if (err) {
    ...
    try {
      ...
      const artworkRecords = files.map(file => ({
        kitId,
        imagePath: `/uploads/kits/${file.filename}`
      }));
      
      // Insert all artwork records into database
      const newArtwork = [];
      for (const record of artworkRecords) {
        const validatedData = insertKitArtworkSchema.parse(record);
        const [artwork] = await db.insert(kitArtwork).values(validatedData).returning();
        newArtwork.push(artwork);
      }
