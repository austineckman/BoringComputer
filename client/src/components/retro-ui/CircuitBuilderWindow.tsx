...
type ComponentType = 'battery' | 'led' | 'resistor' | 'breadboard' | 'switch' | 'potentiometer' | 'capacitor' | 'microcontroller' | 'wire';
...
`;
  ...
  // Analyze MicroPython code to extract pin states
  const analyzeMicroPythonCode = (code: string): Record<string, 'HIGH' | 'LOW'> => {
    const pinStates: Record<string, 'HIGH' | 'LOW'> = {};
    ...
    try {
      // Look for pin initialization patterns
      const pinRegex = /Pin\s*\(\s*(\d+|\w+)\s*,\s*Pin\.OUT\s*\)/g;
      const pinMatches = Array.from(code.matchAll(pinRegex));
      
      // Process each pin initialization
      pinMatches.forEach(match => {
        const pinNumber = match[1];
        let pinName = '';
        
        // Convert to GPx format if it's a number
        if (!isNaN(Number(pinNumber))) {
          pinName = `GP${pinNumber}`;
        } else {
          // This could be a variable, try to find its value
          const varRegex = new RegExp(`${pinNumber}\\s*=\\s*(\\d+)`, 'g');
          const varMatch = Array.from(code.matchAll(varRegex))[0];
          if (varMatch) {
            pinName = `GP${varMatch[1]}`;
          }
        }
        
        // If we found a valid pin name, check for HIGH/LOW states
        if (pinName) {
          // Check for .on() calls
          const onRegex = new RegExp(`(${pinNumber}|${pinName.replace('GP', '')})\\s*\\.\\s*on\\s*\\(\\s*\\)`, 'g');
          if (code.match(onRegex)) {
            pinStates[pinName] = 'HIGH';
          }
          
          // Check for .value(1) calls (HIGH)
          const valueHighRegex = new RegExp(`(${pinNumber}|${pinName.replace('GP', '')})\\s*\\.\\s*value\\s*\\(\\s*1\\s*\\)`, 'g');
          if (code.match(valueHighRegex)) {
            pinStates[pinName] = 'HIGH';
          }
          
          // Check for .off() calls (LOW) - these would override previous HIGH settings
          const offRegex = new RegExp(`(${pinNumber}|${pinName.replace('GP', '')})\\s*\\.\\s*off\\s*\\(\\s*\\)`, 'g');
          if (code.match(offRegex)) {
            pinStates[pinName] = 'LOW';
          }
          
          // Check for .value(0) calls (LOW) - these would override previous HIGH settings
          const valueLowRegex = new RegExp(`(${pinNumber}|${pinName.replace('GP', '')})\\s*\\.\\s*value\\s*\\(\\s*0\\s*\\)`, 'g');
          if (code.match(valueLowRegex)) {
            pinStates[pinName] = 'LOW';
          }
        }
      });
      
      // Also look for direct high-level pin declarations with initial state
      const directPinRegex = /(\w+)\s*=\s*Pin\s*\(\s*(\d+|\w+)\s*,\s*Pin\.OUT\s*,\s*value\s*=\s*(0|1)\s*\)/g;
      const directPinMatches = Array.from(code.matchAll(directPinRegex));
      
      directPinMatches.forEach(match => {
        const varName = match[1];
        const pinNumber = match[2];
        const value = match[3];
        ...
        if (pinName) {
          pinStates[pinName] = value === '1' ? 'HIGH' : 'LOW';
        }
      });
      
    } catch (error) {
      console.error('Error analyzing MicroPython code:', error);
    }
